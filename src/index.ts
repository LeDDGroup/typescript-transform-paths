...
const transformer = (_: ts.Program, config?: PluginConfig) => (context: ts.TransformationContext) => (sourceFile: ts.SourceFile) => {
  ...
  function unpathExportDeclaration(node: ts.ExportDeclaration): ts.VisitResult<ts.Statement> {
    ...
    return ts.updateExportDeclaration(node, node.decorators, node.modifiers, node.exportClause, fileLiteral);
  }
