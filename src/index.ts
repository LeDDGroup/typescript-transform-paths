...
export default function transformer(program: ts.Program, config: PluginConfig & TsTransformPathsConfig) {
  ...
  return (context: ts.TransformationContext) => (sourceFile: ts.SourceFile) => {
    ...
    function update(original: ts.Node, moduleName: string, updaterFn: (newPath: ts.StringLiteral) => ts.Node): ts.Node {
      ...
      const { resolvedModule, failedLookupLocations } = ts.resolveModuleName(moduleName, fileName, compilerOptions, ts.sys);
    ...
    function visit(node: ts.Node): ts.Node | undefined {
      if (isRequire(node) || isAsyncImport(node))
        return update(node, (<ts.StringLiteral>node.arguments[0]).text, p => ts.updateCall(
          node,
          node.expression,
          node.typeArguments,
          [ p ]
      ...
      if (ts.isExternalModuleReference(node) && ts.isStringLiteral(node.expression))
        return update(node, node.expression.text, p => ts.updateExternalModuleReference(node, p));
      ...
      if (ts.isImportDeclaration(node) && ts.isStringLiteral(node.moduleSpecifier))
        return update(node, node.moduleSpecifier.text, p =>
          ...
          Object.assign(node, { moduleSpecifier: ts.updateNode(p, node.moduleSpecifier) })
        );
      ...
      if (ts.isImportTypeNode(node)) {
        ...
        return !text ? node : update(node, text, p => ts.updateImportTypeNode(
          node,
