...
const makeRelative = (fileName: string, p: string, rootDir: string) => {
  let rel = ts.normalizePath(path.relative(path.dirname(fileName), path.join(rootDir, p)));
  if (rel[0] !== ".") rel = `./${rel}`;
  return `"${rel}"`;
};

const getExpected = (fileName: string, original: string, rootDir: string): string =>
  original
    .replace(/"@(.*)"/g, (_, p) => makeRelative(fileName, p, rootDir))
    .replace(/"#utils\/(.*)"/g, (_, p) =>
      makeRelative(fileName, path.join(p === "hello" ? "secondary" : "utils", p), rootDir)
    )
...
describe(`Transformer`, () => {
  describe("Bulk files", () => {
    ...
    describe.each(fileNames.map((p) => [p.slice(bulkPath.length), p]))(`%s`, (_, file) => {
  ...
  describe("Specific tests", () => {
    ...
    const tsConfig = ts.normalizePath(path.join(fixturesPath, "specific/tsconfig.json"));
    const genFile = ts.normalizePath(path.join(specificPath, "generated/dir/gen-file.ts"));
    const srcFile = ts.normalizePath(path.join(specificPath, "src/dir/src-file.ts"));
    const indexFile = ts.normalizePath(path.join(specificPath, "src/index.ts"));
    const typeElisionIndex = ts.normalizePath(path.join(specificPath, "src/type-elision/index.ts"));
