...
export function getFirstOriginal<T extends Node>(node: T): T {
  for (let n: any = node; n; n = n.original) if (!n.original) return n;
...
export function getString({ ts, printer }: UnderscoreTestContext, node: Node, sourceFile: SourceFile): string {
  const res: string | undefined =
    ts.isStringLiteral(node) || ts.isNoSubstitutionTemplateLiteral(node) ? node.text : void 0;

  if (!res) throw new Error(`Expression is not a string! Expression: ${getFirstOriginal(node).getText(sourceFile)}`);
  return res;
}

export function getStringOrFunctionOrObjectLiteral<T>(
  ctx: UnderscoreTestContext,
  node: Node,
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `Expression is not a valid string, function, or object literal. Expression ${getFirstOriginal(node).getText(
          sourceFile
        )}`
      );
    }
...
export function getStringOrFunction<T>({ ts, printer }: UnderscoreTestContext, node: Node, sourceFile: SourceFile): T {
  const res: string | Function | undefined =
    ts.isStringLiteral(node) || ts.isNoSubstitutionTemplateLiteral(node)
      ...
      ? (eval(`(${printer.printNode(ts.EmitHint.Expression, node, sourceFile)})`) as Function)
      : void 0;

  if (!res)
    throw new Error(
      `Expression is not a string or function! Expression: ${getFirstOriginal(node).getText(sourceFile)}`
    );
...
export function getObjectLiteral<T>({ ts, printer }: UnderscoreTestContext, node: Node, sourceFile: SourceFile): T {
  ...
  if (!res)
    throw new Error(`Expression is not an object literal! Expression: ${getFirstOriginal(node).getText(sourceFile)}`);
  return res as unknown as T;
}
